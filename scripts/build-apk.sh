...
logo=os.environ.get('LOGO_DATA',''); ext=''; raw=''
if logo.startswith('data:image/') and ';base64,' in logo:
 m,p=logo.split(';base64,',1); ext='png' if 'png' in m else ('webp' if 'webp' in m else 'jpg'); raw=base64.b64decode(p); (r/f'app/src/main/res/drawable-nodpi/app_logo.{ext}').write_bytes(raw); icon='@drawable/app_logo'
else:
...
# Visible in-app logo + app name
logo_view='';
if ext: logo_view='<ImageView android:id="@+id/appLogo" android:layout_width="72dp" android:layout_height="72dp" android:layout_gravity="center_horizontal" android:src="@drawable/app_logo" android:contentDescription="App logo" />'
(r/'app/src/main/res/layout_main.xml').write_text('') if False else None
...
java='package com.appcreator; import android.app.*; import android.os.*; import android.graphics.Color; import android.view.*; import android.webkit.*; import android.widget.*; public class MainActivity extends Activity { public void onCreate(Bundle b){super.onCreate(b); LinearLayout r=new LinearLayout(this);r.setOrientation(LinearLayout.VERTICAL); TextView t=new TextView(this);t.setText(R.string.app_name);t.setTextColor(Color.WHITE);t.setTextSize(19);t.setPadding(16,8,8,8);t.setBackgroundColor(Color.rgb(23,32,51));r.addView(t);'
...
PY
(cd "$root" && gradle --no-daemon --offline clean assembleRelease)
