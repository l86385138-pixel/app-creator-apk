...
# A scheduled worker may find an older row stuck in building after a cancelled runner. Recover it.
if [ -n "$BUILD_ID" ]; then
 ...
 for i in $(seq 1 24); do
  ...
  # Recover builds left in building for more than 15 minutes.
  stale=$(curl -fsS "${headers[@]}" "$api/app_creator_builds?select=id,project_id&status=eq.building&created_at=lt.$(date -u -d '15 minutes ago' +%Y-%m-%dT%H:%M:%SZ)&order=created_at.asc&limit=1")
  if [ "$stale" != "[]" ]; then
...
KEYSTORE_PASSWORD="${APK_KEYSTORE_PASSWORD:-AppCreatorRelease2026!}"; KEY_PASSWORD="${APK_KEY_PASSWORD:-$KEYSTORE_PASSWORD}"; KEY_ALIAS="${APK_KEY_ALIAS:-appcreator}"
