...
java='package com.appcreator; import android.app.*; import android.os.*; import android.graphics.Color; import android.view.Gravity; import android.webkit.*; import android.widget.*; public class MainActivity extends Activity { private int dp(int v){ return (int)(v*getResources().getDisplayMetrics().density+0.5f); } public void onCreate(Bundle b){super.onCreate(b); LinearLayout r=new LinearLayout(this);r.setOrientation(LinearLayout.VERTICAL); LinearLayout header=new LinearLayout(this); header.setOrientation(LinearLayout.HORIZONTAL); header.setGravity(Gravity.CENTER_VERTICAL); header.setPadding(dp(12),dp(8),dp(12),dp(8)); header.setBackgroundColor(Color.WHITE); ImageView logo=new ImageView(this); logo.setImageResource(com.appcreator.R.drawable.app_logo); logo.setScaleType(ImageView.ScaleType.CENTER_CROP); LinearLayout.LayoutParams lpLogo=new LinearLayout.LayoutParams(dp(42),dp(42)); lpLogo.gravity=Gravity.CENTER_VERTICAL; header.addView(logo,lpLogo); TextView t=new TextView(this);t.setText(R.string.app_name);t.setTextColor(Color.rgb(23,32,51));t.setTextSize(18);t.setGravity(Gravity.CENTER_VERTICAL);t.setMaxLines(1);t.setEllipsize(android.text.TextUtils.TruncateAt.END);t.setPadding(dp(12),0,0,0); LinearLayout.LayoutParams lpTitle=new LinearLayout.LayoutParams(0,dp(58),1f); lpTitle.gravity=Gravity.CENTER_VERTICAL; header.addView(t,lpTitle); r.addView(header,new LinearLayout.LayoutParams(-1,dp(58))); WebView w=new WebView(this);w.getSettings().setJavaScriptEnabled(true);w.getSettings().setDomStorageEnabled(true);w.setWebViewClient(new WebViewClient());w.loadUrl("'+u+'");r.addView(w,new LinearLayout.LayoutParams(-1,0,1));setContentView(r);}}'
